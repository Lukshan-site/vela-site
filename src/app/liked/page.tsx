...
export default async function LikedPage() {
  ...
  const { data: rows } = await supabase
    .from("likes").select("video_id, created_at").eq("user_id", user.id).order("created_at", { ascending: false });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted text-sm mt-1">All videos you&apos;ve liked.</p>
      </div>
      <VideoGrid videos={videos} empty="No liked videos yet. Hit the Like button on any video!" />
    </div>
  );
}
